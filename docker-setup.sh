...
# This file is used to automatically download required files and create directories
# Currently only supports X86 Ubuntu systems, other systems are untested
...
echo -e "\e[1;36m  Xiaozhi Server Full Deployment One-Click Installation Script Ver 0.2 Updated Aug 20, 2025 \e[0m\n"
sleep 1
...
if check_installed; then
    if whiptail --title "Installation Detected" --yesno "Xiaozhi Server is already installed. Do you want to upgrade?" 10 60; then
        ...
        # Stop and remove specific containers (considering containers might not exist)
        containers=(
        ...
        docker compose -f /opt/xiaozhi-server/docker-compose_all.yml up -d
          ...
          # Skip upgrade, continue with installation flow
    fi
fi
...
if ! command -v docker &> /dev/null; then
    ...
    # Use China mirror instead of official source
...
case $MIRROR_CHOICE in
    ...
    7) MIRROR_URL=$(whiptail --title "Custom Mirror" --inputbox "Please enter complete mirror URL:" 10 60 3>&1 1>&2 2>&3) ;; 
    8) MIRROR_URL="" ;; 
esac
...
if [ ! -f "$MODEL_PATH" ]; then
    ...
    ) | whiptail --title "Downloading" --gauge "Starting voice recognition model download..." 10 60 0
...
# Only download if not upgrade completed
...
if [ $? -ne 0 ]; then
    whiptail --title "Error" --msgbox "Docker service startup failed. Please try changing the mirror and run this script again" 10 60
    exit 1
fi
...
whiptail --title "Configure Server Key" --msgbox "Please use a browser to visit the following link to open the Admin Console and register an account: \n\nLocal address: http://127.0.0.1:8002/\nPublic address: http://$PUBLIC_IP:8002/ (If using a cloud server, please open ports 8000, 8001, 8002 in security groups).\n\nThe first registered user will be the super administrator. Later registered users will be regular users. Regular users can only bind devices and configure agents; Super administrators can perform model management, user management, parameter configuration, etc.\n\nAfter registration, press Enter to continue" 18 70
SECRET_KEY=$(whiptail --title "Configure Server Key" --inputbox "Please login to Admin Console with super administrator account\nLocal address: http://127.0.0.1:8002/\nPublic address: http://$PUBLIC_IP:8002/\nIn top menu Parameter Dictionary → Parameter Management, find parameter code: server.secret (Server Key)\nCopy the parameter value and enter it in the input box below\n\nPlease enter key (leave blank to skip configuration):" 15 60 3>&1 1>&2 2>&3)
...
# Fixed log file ws issue, changed to hardcoded
whiptail --title "Installation Complete!" --msgbox "\
Server related addresses:\n\
Admin Console: http://$LOCAL_IP:8002\n\
